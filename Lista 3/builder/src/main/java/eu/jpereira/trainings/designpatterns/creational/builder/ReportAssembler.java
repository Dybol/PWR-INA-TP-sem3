...
public class ReportAssembler {
	...
	public Report getReport(ReportBuilder reportBuilder) {
		...
		reportBuilder.setName(saleEntry.getCustomer().getName());
		reportBuilder.setPhone(saleEntry.getCustomer().getPhone());
		reportBuilder.startItems();
		for(Iterator<SoldItem> it = saleEntry.getSoldItems().iterator(); it.hasNext();) {
			SoldItem item = it.next();
			reportBuilder.addItem(item.getName(), item.getQuantity(), item.getUnitPrice());
		}
